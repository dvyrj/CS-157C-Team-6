...
router.post('/api/authenticate_emailid', async (req, res) => {

  const credentials = req.body;
  console.log('Fetching Details from DB');
  const fetch_user = `SELECT * FROM users where user_emailId = ? ALLOW FILTERING`;
  await CassandraClient.execute(fetch_user, [credentials.emailId],async function (error, results) {
    if(error) {
      console.log(error);
    }
    else {
    ...
    if (results.length > 1) {
      console.log("Multiple Entries exists, PLease Delete one user");
      res.writeHead(301, {
        'Content-Type': 'text/plain'
      });
      res.send("Duplicate Entries Found, Please Contact the System Admin");
    }
      ...
      res.writeHead(400, 'Contact System Admin', {
        'Content-Type': 'text/plain',
      });
      res.end();
    }
      ...
      res.writeHead(500, 'Contact System Admin', {
        'Content-Type': 'text/plain',
      });
      res.end();
    }
